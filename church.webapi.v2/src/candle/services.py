...
#region candle_type
def create_candle_type():
    try:
        data = request.get_json()
        new_candle_type = CandleType(**data)
        db.session.add(new_candle_type)
        db.session.commit()
        return jsonify({"message": "CandleType создан", "data": CandleTypeSchema().dump(new_candle_type)}), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 400
    

def get_all_candle_types():
    try:
        candle_types = CandleType.query.all()
        return jsonify({"data": candles_type_schema.dump(candle_types)}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500


def get_candle_type(candle_type_id):
    try:
        candle_type = CandleType.query.get(candle_type_id)
        if not candle_type:
            return jsonify({"message": "CandleType не найден"}), 404
        return jsonify({"data": CandleTypeSchema().dump(candle_type)}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500


def delete_candle_type(candle_type_id):
    try:
        candle_type = CandleType.query.get(candle_type_id)
        if not candle_type:
            return jsonify({"message": "CandleType не найден"}), 404

        db.session.delete(candle_type)
        db.session.commit()
        return jsonify({"message": "CandleType удален"}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500


def update_candle_type(candle_type_id):
    try:
        data = request.get_json()
        candle_type = CandleType.query.get(candle_type_id)
        if not candle_type:
        ...
        for key, value in data.items():
            setattr(candle_type, key, value)

        db.session.commit()
        return jsonify({"message": "CandleType обновлен", "data": CandleTypeSchema().dump(candle_type)}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 400
